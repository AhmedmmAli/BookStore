...
def index(request):
    if request.user.is_authenticated:
        ...
        userFollow = Author.objects.filter(follow=request.user)

        categories = Category.objects.all()

        return render(request, 'home.html', { 'MostReadBooks' : MostReadBooks ,'userFollow':userFollow ,'topRatedBooks': topRatedBooks, 'topFollowedAuthors' : topFollowedAuthors, 'categories': categories,'userBooks':userBooks,'userWishs':userWishs,'userCategories':userCategories})
    else:
...
def getBook(request, book_id):
    if request.user.is_authenticated:
        ...
        if request.method == 'GET':
            ...
            return render(request, 'book.html', {'book': book, 'show_read': show_read, 'show_wish': show_wish, 'rateVal':rateVal})
        else:
            # book_id
...
def status(request,rateVal,book_id):
    try:
        rate=Rate.objects.get(book_id=book_id,user_id = request.user.id)
        rate.rate=rateVal
        rate.save()
    except Exception as e:
        Rate.objects.create(rate=rateVal,book_id=book_id, user_id = request.user.id)
    return redirect('/book/'+str(book_id))
